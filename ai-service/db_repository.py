...
def get_inspection_by_id(inspection_id: str) -> Optional[Dict[str, Any]]:
    ...
    if _use_mongo and _mongo_db is not None:
        try:
            doc = _mongo_db.inspections.find_one({
                "$or": [{"inspection_id": inspection_id}, {"id": inspection_id}]
            })
            if doc:
                doc.pop("_id", None)
                return doc
    ...
    items = _read_json_data()
    for item in items:
        if item.get("inspection_id") == inspection_id or item.get("id") == inspection_id:
            return item
    return None
...
def delete_inspection(inspection_id: str) -> bool:
    ...
    if _use_mongo and _mongo_db is not None:
        try:
            res = _mongo_db.inspections.delete_one({
                "$or": [{"inspection_id": inspection_id}, {"id": inspection_id}]
            })
    ...
    items = _read_json_data()
    orig_len = len(items)
    items = [it for it in items if it.get("inspection_id") != inspection_id and it.get("id") != inspection_id]
...
def get_dashboard_stats() -> Dict[str, Any]:
    ...
    inspections_data = get_inspections(limit=1000)
