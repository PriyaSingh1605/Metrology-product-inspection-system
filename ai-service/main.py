...
def list_inspections(
    ...
    """Retrieve paginated inspection records with search and status filtering."""
    return repo.get_inspections(search=search, status_filter=status, page=page, limit=limit)


@app.get("/api/inspections/{inspection_id}")
def get_inspection(inspection_id: str):
    """Retrieve full details of an inspection by ID."""
    doc = repo.get_inspection_by_id(inspection_id)
    if not doc:
...
def delete_inspection(inspection_id: str):
    ...
    deleted = repo.delete_inspection(inspection_id)
    if not deleted:
        raise HTTPException(status_code=404, detail=f"Inspection '{inspection_id}' not found.")
...
def download_inspection_pdf(inspection_id: str):
...
def dashboard_stats():
    """Retrieve dashboard statistics and violation trends."""
