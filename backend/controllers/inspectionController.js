...
exports.listInspections = async (req, res, next) => {
  try {
    ...
    // Officers only see their own inspections. Admins can see all inspections.
    if (req.user?.role !== 'admin') params.user_id = req.user.sub;
...
exports.getInspection = async (req, res, next) => {
  try {
    ...
    const doc = await getInspectionById(req.params.id, params);
    if (!doc) {
...
exports.deleteInspection = async (req, res, next) => {
  try {
    const params = req.user?.role === 'admin' ? {} : { user_id: req.user.sub };
    const result = await deleteInspection(req.params.id, params);
    if (!result) {
      return res.status(404).json({ error: `Inspection '${req.params.id}' not found.` });
    }
...
exports.downloadPdf = async (req, res, next) => {
  try {
    // PDF access follows the same ownership rule as inspection details.
    const params = req.user?.role === 'admin' ? {} : { user_id: req.user.sub };
    const pdfResponse = await streamPdfReport(req.params.id, params);
    if (!pdfResponse) {
      return res.status(404).json({ error: `Inspection '${req.params.id}' not found.` });
    }
